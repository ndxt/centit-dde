	...
	$.extend($.fn, {
				...
				$(op.selector, box).click(function(){
					...
					if (options.is(":hidden")) {
						if(options.height() > 300) {
							options.css({height:"300px",width:options.width()+20+"px",overflow:"auto"});
						}
		...
		combox:function(){
			...
			return this.each(function(i){
				...
				var width = parseInt($this.attr("width"))+"px";
				...
				select += '<a href="javascript:" class="'+$this.attr("class")+'" name="' + name +'" value="' + value + '" style="width:'+width+'">' + label +'</a></div></div>';
