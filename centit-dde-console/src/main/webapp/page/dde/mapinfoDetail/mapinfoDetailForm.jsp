...
<div style="float:left">
	<dl class="nowrap">
		<dt>左（数据源）定义：</dt>
		<dd>
		    <input name="s_sourceDatabaseName" type="text" size="10"
  ...
  <div style="float:left">
    ...
    <fieldset style="width:500px;">
     ...
     <dl class="nowrap">
				...
				<input type="radio" name="recordOperate" value="1" class="recordOperate_edit" <c:if test="${recordOperate eq '1'}">checked=true</c:if> />插入（insert）<br/>
				<input type="radio" name="recordOperate" value="2" class="recordOperate_edit" <c:if test="${recordOperate eq '2'}">checked=true</c:if>/>更新（update）<br/>
				<input type="radio" name="recordOperate" value="3" class="recordOperate_edit" <c:if test="${recordOperate eq '3'}">checked=true</c:if>/>合并（merge）
				</dd>
	</dl>
   </fieldset>
    <dl class="nowrap" style="display: inline;">
				<dt>是否重复执行：</dt>
				<dd>
				<input type="radio" name="isRepeat" value="1" class="isRepeat_edit" <c:if test="${isRepeat eq '1'}">checked=true</c:if> />是
				<input type="radio" name="isRepeat" value="0" class="isRepeat_edit" <c:if test="${isRepeat eq '0'}">checked=true</c:if>/>否
				</dd>
	</dl>
  </div>
</div>
    ...
    <div class="tabs tabsExtra">
		...
		<div class="tabsContentAjax">
			<div class="unitBox" id="jbsxBox">
				<%@ include file="/page/dde/mapInfo4Details.jsp"%>
			</div>
		</div>
