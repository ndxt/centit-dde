...
var navTab = {
	...
	_op: {container:"#container", header:'#header', leftside:'#leftside', splitBar:'#splitBar', footer:'#footer', id:"navTab", stTabBox:".navTab-tab", stPanelBox:".navTab-panel", mainTabId:"main", close$:"a.close", prevClass:"tabsLeft", nextClass:"tabsRight", stMore:".tabsMore", stMoreLi:"ul.tabsMoreList"},
	...
	init: function(options){
		...
		this._makeFulllScreen();
	},
	_makeFulllScreen: function() {
		var $this = this, header = $(this._op.header), leftside = $(this._op.leftside), splitBar = $(this._op.splitBar),
				container = $(this._op.container), footer = $(this._op.footer);
		
		$(this._op.stTabBox + ' > li').live('dblclick', function() {
			...
			if (!$this.isFullScreen) {
				...
				container.find($this._op.stPanelBox).height(height - 28);
			}
			// 取消全屏
			else {
				...
				container.css($this.containerInfo);
	...
	gotoTab: function(navtabId) {
		navTabAjaxDone({'statusCode' : 200, 'callbackType' : 'closeCurrent', 'navTabId' : navtabId});
	},
