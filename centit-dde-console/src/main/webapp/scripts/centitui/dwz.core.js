	...
	$.fn.extend({
		...
		layoutH: function($refBox){
			return this.each(function(){
				...
				var parent = $this.parents("div.page:first");
				
				// 对于隐藏的TAB页内，不需要进行自适应高度
				if (parent[0] && parent.is(':hidden')) {
					return true;
				}
...
//						console.log('height', parent.find(selector).outerHeight(true));
//						console.log('iLayoutH', iLayoutH);
//					}
//					else {
//						alert('height ' + parent.find(selector).outerHeight(true));
//						alert('iLayoutH ' + iLayoutH);
//					}
				}
...
//				}
		...
		/**
		 * 返回元素容器是NAVTAB还是DIALOG
		 * @param obj
		 * @return
		 */
		getContainer: function() {
			var obj = $(this);
			var container = obj.closest('.dialog')[0] || obj.closest('.unitBox')[0];
