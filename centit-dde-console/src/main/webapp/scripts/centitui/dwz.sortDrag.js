		...
		drag:function(){
			...
			if ($overBox.length > 0 && $overBox[0] != $sortBox[0]){ //移动到其他容器
				...
				for (var i=0; i<$items.length; i++) {
					...
					if (parseInt($helper.css("top")) > position.top + 5) {
						$this.after($placeholder);
					} else if (parseInt($helper.css("top")) <= position.top) {
		...
		stop:function(){
			...
			$helper.animate({
				...
				complete: function(){
					...
					if($op.formId){
						var _formId = "#"+$op.formId;
						$(_formId).submit();
					}
					
					$op.callback($srcBox, $destBox);
				},
				duration: 300
			});
		},
		save:function(){
			var $helper = $(arguments[0]),$op = $helper.data('op');
			if(op.formId!=''||op.formId!=undefined){
				var _formId = "#"+op.formId;
				$(_formId).submit();
			}
		},
