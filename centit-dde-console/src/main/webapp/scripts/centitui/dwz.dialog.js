...
(function($){
	$.pdialog = {
		_op:{height:300, width:580, minH:40, minW:50, total:20, max:false, mask:true, resizable:true, drawable:true, maxable:false,minable:false,fresh:true,shadow:'div#dwzShadow'},
		_current:null,
		_zIndex:42,
		getCurrent:function(){
		...
		open:function(url, dlgid, title, options) {
			...
			if(dialog) {
				...
				$(this._op.shadow).css("zIndex", $.pdialog._zIndex - 3).show();
...
//					$.pdialog.switchDialog(dialog);
		...
		switchDialog:function(dialog) {
			...
			if($.pdialog._current) {
				...
				$(this._op.shadow).css("zIndex", cindex - 1);
				$.pdialog._current = dialog;
			}
		...
		attachShadow:function(dialog) {
			...
			if ($.browser.msie) {
				...
				shadow.css({
					top: parseInt($(dialog)[0].style.top) + 2,
					left: parseInt($(dialog)[0].style.left) + 2,
					height: parseInt($(dialog).height()) - 2,
					width: parseInt($(dialog).width()) - 2,
					zIndex:parseInt($(dialog).css("zIndex")) - 1
				});
			}
