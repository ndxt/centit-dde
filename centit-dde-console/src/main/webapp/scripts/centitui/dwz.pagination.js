	...
	$.fn.extend({
		...
		orderBy: function(options){
			...
			return this.each(function(){
				...
				if (op.rel) {
					var form = $("#pagerForm", $parent.find('#'+op.rel)).get(0);
				}
				else {
					var form = $("#pagerForm", $parent).get(0);
				}
				
				var orderFieldInput = form.orderField;
			    var orderDirectionInput = form.orderDirection;		   
				...
				if ($this.attr('orderField') == orderFieldInput.value) {
					$this.addClass(orderDirectionInput.value);
				}
			});
		},
